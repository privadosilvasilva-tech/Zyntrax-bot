...
class MembrosRoleSelect(discord.ui.RoleSelect):
    def __init__(self, parent_view: "PainelAntiLinkView"):
        super().__init__(placeholder="🔍 Digite para filtrar cargos de MEMBROS", min_values=0, max_values=25)
        self.parent_view = parent_view
    
    async def callback(self, interaction: discord.Interaction):
        self.parent_view.allowed_roles = {role.id for role in self.values}
        embed = self.parent_view.montar_embed()
        await interaction.response.edit_message(embed=embed)

class BotsRoleSelect(discord.ui.RoleSelect):
    def __init__(self, parent_view: "PainelAntiLinkView"):
        super().__init__(placeholder="🔍 Digite para filtrar cargos de BOTS", min_values=0, max_values=25)
        self.parent_view = parent_view
    
    async def callback(self, interaction: discord.Interaction):
        self.parent_view.allowed_bot_roles = {role.id for role in self.values}
        embed = self.parent_view.montar_embed()
        await interaction.response.edit_message(embed=embed)
...
class PainelAntiLinkView(discord.ui.View):
    ...
    async def membros_roles_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        select = MembrosRoleSelect(self)
        view = discord.ui.View()
        view.add_item(select)
        await interaction.response.send_message("👥 **Selecione cargos de MEMBROS que podem enviar convites:**", view=view, ephemeral=True)
    ...
    async def bots_roles_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        select = BotsRoleSelect(self)
        view = discord.ui.View()
        view.add_item(select)
        await interaction.response.send_message("🤖 **Selecione cargos de BOTS que podem enviar convites:**", view=view, ephemeral=True)
