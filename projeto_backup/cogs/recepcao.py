...
def montar_meta_texto(session: dict, tipo: str) -> str:
    ...
    return (
        f"🔧 **Configurar {nome_tipo}**\n"
        f"Status: {status} • Canal: {canal} • Marcar membro no conteúdo: {ping}\n"
        f"-# Pré-visualização abaixo usa seus próprios dados como exemplo."
    )
...
class EntradaSaidaView(BaseView):
    ...
    async def atualizar(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(
            content=montar_meta_texto(session, self.tipo), embed=preview, view=self
        )
    ...
    async def cor_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        preview = montar_preview(session, interaction.user, interaction.guild)
        await interaction.response.edit_message(content=montar_meta_texto(session, self.tipo), embed=preview, view=view)
    ...
    async def salvar_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        save_config(config)
        await interaction.response.edit_message(
            content=f"✅ Configuração de {self.tipo} salva!\n\n" + montar_meta_texto(session, self.tipo),
            embed=montar_preview(session, interaction.user, interaction.guild),
            view=self,
        )
...
class VoltarParaEditarButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        preview = montar_preview(session, interaction.user, interaction.guild)
        await interaction.response.edit_message(
            content=montar_meta_texto(session, self.parent_view.tipo), embed=preview, view=self.parent_view
        )
...
class RecepcaoPainelView(BaseView):
    ...
    def texto_painel(self) -> str:
        return (
            "📥 **Configurar Recepção**\n"
            "Escolha o que quer configurar — cada um tem canal, cor, imagens e texto próprios."
        )
    ...
    async def entrada_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        session = BUILD_SESSIONS[key]
        preview = montar_preview(session, interaction.user, interaction.guild)
        await interaction.response.edit_message(content=montar_meta_texto(session, "entrada"), embed=preview, view=view)
    ...
    async def saida_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        session = BUILD_SESSIONS[key]
        preview = montar_preview(session, interaction.user, interaction.guild)
        await interaction.response.edit_message(content=montar_meta_texto(session, "saida"), embed=preview, view=view)
