...
def montar_meta_texto(tipo: dict) -> str:
    status = "🟢 Ativo" if tipo.get("ativo") else "🔴 Inativo"
    canais = []
    canais.append(f"Envio: <#{tipo['canal_envio']}>" if tipo.get("canal_envio") else "Envio: `-`")
    canais.append(f"Análise: <#{tipo['canal_analise']}>" if tipo.get("canal_analise") else "Análise: `-`")
    canais.append(f"Resultados: <#{tipo['canal_resultados']}>" if tipo.get("canal_resultados") else "Resultados: `-`")
    ...
    return (
        f"🔧 **Formulário: {tipo['nome']}** — {status}\n"
        f"{' • '.join(canais)}\n"
        f"Cargo ao aprovar: {cargo}\n"
        f"Revisores: {revisores}\n"
        f"Perguntas: `{len(tipo.get('perguntas', []))}`"
    )
...
class RevisoresView(BaseView):
    ...
    async def atualizar(self, interaction: discord.Interaction):
        tipo = BUILD_SESSIONS[self.key]
        await interaction.response.edit_message(content=montar_meta_texto(tipo), embed=montar_preview(tipo), view=self)
    ...
    async def voltar_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        tipo = BUILD_SESSIONS[self.key]
        await interaction.response.edit_message(content=montar_meta_texto(tipo), embed=montar_preview(tipo), view=self.parent_view)
...
class TipoBuilderView(BaseView):
    ...
    async def cor_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(content=montar_meta_texto(tipo), embed=montar_preview(tipo), view=view)
    ...
    async def salvar_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(
            content=f"✅ **{tipo['nome']}** salvo!\n\n" + montar_meta_texto(tipo), embed=montar_preview(tipo), view=self
        )
    ...
    async def publicar_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        self._persistir(interaction.guild_id)
        await interaction.response.edit_message(
            content=f"✅ Publicado em {canal.mention} e ativado!\n\n" + montar_meta_texto(tipo),
            embed=montar_preview(tipo),
            view=self,
        )
    ...
    async def voltar_final_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(content=await view.texto_painel(interaction.guild_id), embed=None, view=view)
...
class FormulariosPainelView(discord.ui.View):
    ...
    async def texto_painel(self, guild_id: int) -> str:
        config = load_config()
        tipos = get_guild_tipos(config, guild_id)
        if not tipos:
            return "📋 **Formulários**\nNenhum formulário criado ainda. Toque em \"➕ Criar Novo\"."
    ...
    async def editar_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(content=await self.texto_painel(interaction.guild_id), view=view)
