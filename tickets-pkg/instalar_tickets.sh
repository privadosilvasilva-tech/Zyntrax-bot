...
# Este script cria/atualiza automaticamente os arquivos necessarios.
...
echo "📦 Instalando o sistema de Tickets (Zyntrax SmartDesk)..."
...
class TicketsPainelView(discord.ui.View):
    ...
    async def texto_painel(self, guild_id: int) -> discord.Embed:
        ...
        transcripts_txt = f"<#{gconf['canal_transcripts']}>" if gconf.get("canal_transcripts") else "*(não definido)*"
        ...
        embed.add_field(name="🗂️ Canal de transcripts", value=transcripts_txt, inline=True)
    ...
    @discord.ui.button(label="🚀 Publicar Painel Aqui", style=discord.ButtonStyle.success, row=1)
    async def publicar(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        gconf["canal_painel"] = interaction.channel.id
    ...
    @discord.ui.button(label="⚙️ Limites e Cooldown", style=discord.ButtonStyle.secondary, row=2)
    ...
    @discord.ui.select(cls=discord.ui.ChannelSelect, channel_types=[discord.ChannelType.category],
                        placeholder="📁 Categoria padrão do Discord (fallback)", row=3)
    ...
    @discord.ui.select(cls=discord.ui.ChannelSelect, channel_types=[discord.ChannelType.text],
                        placeholder="🗂️ Canal para enviar transcripts", row=4)
    async def escolher_canal_transcripts(self, interaction: discord.Interaction, select: discord.ui.ChannelSelect):
        config = load_config()
        gconf = get_guild_config(config, interaction.guild_id)
        gconf["canal_transcripts"] = select.values[0].id
        save_config(config)
        await interaction.response.edit_message(embed=await self.texto_painel(interaction.guild_id), view=self)
...
async def enviar_transcript(guild: discord.Guild, canal: discord.TextChannel, ticket: dict, gconf: dict) -> None:
    canal_transcripts_id = gconf.get("canal_transcripts")
    if not canal_transcripts_id:
        return
    destino = guild.get_channel(canal_transcripts_id)
    if not destino:
        return
...
echo "  -> escrevendo cogs/configuracoes.py"
cat > cogs/configuracoes.py << 'ZYNTRAX_EOF_COGS_CONFIGURACOES.PY'
import discord
from discord import app_commands
...
ZYNTRAX_EOF_COGS_CONFIGURACOES.PY

echo "🔎 Verificando sintaxe..."
python3 -m py_compile cogs/*.py && echo "✅ Sintaxe OK"

echo "✅ Sistema de Tickets instalado com sucesso!"
echo "   Reinicie o bot (pkill -f bot.py && python3 bot.py) e use /configuracoes -> 🎫 Tickets"
