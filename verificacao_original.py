...
def montar_meta_texto(sessao: dict) -> str:
    ...
    return (
        f"🔧 **Sistema de Verificação** — {status}\n"
        f"Painel: {canal_v} • Log: {canal_l}\n"
        f"Cargo ao verificar: {cargo_v}\n"
        f"Cargo \"Não Verificado\" (auto no join): {cargo_nv}"
    )
...
class VerificacaoBuilderView(BaseView):
    ...
    async def atualizar(self, interaction: discord.Interaction):
        sessao = BUILD_SESSIONS[self.key]
        await interaction.response.edit_message(content=montar_meta_texto(sessao), embed=montar_preview(sessao, interaction.guild), view=self)
    ...
    async def cor_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(content=montar_meta_texto(sessao), embed=montar_preview(sessao, interaction.guild), view=view)
    ...
    async def salvar_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.edit_message(
            content="✅ Salvo!\n\n" + montar_meta_texto(sessao), embed=montar_preview(sessao, interaction.guild), view=self
        )
